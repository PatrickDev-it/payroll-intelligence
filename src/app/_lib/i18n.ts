...
const IT = {
  ...
  netPerPeriod: "Netto per mensilità · {periods} mensilità",
  ...
  resultAnnouncement: "Netto per mensilità {period}, netto annuo {annual}.",
...
const FR: Record<MessageKey, string> = {
  ...
  netPerPeriod: "Net par paie · {periods} versements", netMonthly: "Net mensuel", methodAndSources: "Méthode et sources", netAnnual: "Net annuel", twelveMonthAverage: "moyenne sur 12 mois", grossRal: "Salaire brut", contractFigure: "montant contractuel", withholdings: "Retenues", employeeTaxesContributions: "impôts et cotisations salariales", employerCost: "Coût employeur", ralMultiplier: "{value} du brut", taxWedge: "Coin fiscal", wedgeHint: "du coût employeur n’atteint pas le net", costSplitTitle: "Répartition du coût employeur", oneHundredOf: "100 % de {value}", costSplitAria: "Répartition du coût employeur de {value} : {segments}", splitNet: "Net du salarié", splitContributions: "Cotisations salariales", splitTaxes: "Impôts", splitEmployer: "Charges employeur",
...
const FIELD_COPY: Record<Exclude<Locale, "it">, Record<string, FieldCopy>> = {
  en: {
    ...
    "DE:countryOptions.children": { label: "Dependent children", short: "Children", help: "Affects long-term-care insurance, including the childless supplement." },
...
const GENERIC_FIELD_LABELS: Record<"de" | "fr" | "es", Record<string, [string, string?]>> = {
  de: {
    ...
    "DE:region": ["Bundesland", "Land"], "DE:countryOptions.steuerklasse": ["Steuerklasse"], "DE:countryOptions.churchMember": ["Kirchensteuer"], "DE:countryOptions.children": ["Unterhaltsberechtigte Kinder", "Kinder"], "DE:countryOptions.zusatzbeitrag": ["Zusatzbeitrag"], "DE:countryOptions.zusatzbeitragRatePercent": ["Genauer Zusatzbeitrag", "Zusatz genau"], "DE:countryOptions.unfallRiskClass": ["Gefahrtarif"], "DE:countryOptions.unfallRatePercent": ["Genauer Unfallsatz", "Unfall genau"], "DE:countryOptions.u2RatePercent": ["Genauer U2-Satz", "U2 genau"],
    ...
    "FR:region": ["Lokales System"], "FR:countryOptions.statut": ["Statut"], "FR:countryOptions.foyer": ["Steuerhaushalt", "Foyer fiscal"], "FR:countryOptions.versementMobilite": ["Versement mobilité"], "FR:countryOptions.versementMobiliteRatePercent": ["Genauer Mobilitätssatz", "Mobilité genau"], "FR:countryOptions.children": ["Unterhaltsberechtigte Kinder", "Kinder"], "FR:countryOptions.atmpRiskClass": ["AT/MP"], "FR:countryOptions.atmpRatePercent": ["Genauer AT/MP-Satz"],
  },
  fr: {
    ...
    "DE:region": ["Land"], "DE:countryOptions.steuerklasse": ["Steuerklasse"], "DE:countryOptions.churchMember": ["Kirchensteuer"], "DE:countryOptions.children": ["Enfants à charge", "Enfants"], "DE:countryOptions.zusatzbeitrag": ["Zusatzbeitrag"], "DE:countryOptions.zusatzbeitragRatePercent": ["Zusatz exact"], "DE:countryOptions.unfallRiskClass": ["Gefahrtarif"], "DE:countryOptions.unfallRatePercent": ["Taux Unfall exact"], "DE:countryOptions.u2RatePercent": ["Taux U2 exact"],
    ...
    "FR:region": ["Régime local"], "FR:countryOptions.statut": ["Statut"], "FR:countryOptions.foyer": ["Foyer fiscal"], "FR:countryOptions.versementMobilite": ["Versement mobilité"], "FR:countryOptions.versementMobiliteRatePercent": ["Taux mobilité exact"], "FR:countryOptions.children": ["Enfants à charge", "Enfants"], "FR:countryOptions.atmpRiskClass": ["AT/MP"], "FR:countryOptions.atmpRatePercent": ["Taux AT/MP exact"],
  },
  es: {
    ...
    "DE:region": ["Land"], "DE:countryOptions.steuerklasse": ["Steuerklasse"], "DE:countryOptions.churchMember": ["Kirchensteuer"], "DE:countryOptions.children": ["Hijos a cargo", "Hijos"], "DE:countryOptions.zusatzbeitrag": ["Zusatzbeitrag"], "DE:countryOptions.zusatzbeitragRatePercent": ["Zusatz exacto"], "DE:countryOptions.unfallRiskClass": ["Gefahrtarif"], "DE:countryOptions.unfallRatePercent": ["Tipo Unfall exacto"], "DE:countryOptions.u2RatePercent": ["Tipo U2 exacto"],
    ...
    "FR:region": ["Régimen local"], "FR:countryOptions.statut": ["Statut"], "FR:countryOptions.foyer": ["Hogar fiscal", "Foyer fiscal"], "FR:countryOptions.versementMobilite": ["Versement mobilité"], "FR:countryOptions.versementMobiliteRatePercent": ["Tipo mobilité exacto"], "FR:countryOptions.children": ["Hijos a cargo", "Hijos"], "FR:countryOptions.atmpRiskClass": ["AT/MP"], "FR:countryOptions.atmpRatePercent": ["Tipo AT/MP exacto"],
  },
};

export const FIELD_HELP_GENERIC: Record<"de" | "fr" | "es", Record<string, string>> = {
  de: { gross: "Jährliches Brutto vor Steuern und Arbeitnehmerbeiträgen; nicht der Gesamtaufwand des Arbeitgebers.", geography: "Dieser Wert bestimmt die örtlich geltenden Steuern, Zuschläge oder Sozialversicherungsregeln.", periods: "Ändert den Betrag je Abrechnung, niemals die Jahressumme.", headcount: "Bestimmt arbeitgeberseitige Sätze und Schwellen nach Unternehmensgröße.", contract: "Die Vertragsart kann die Arbeitgeberbeiträge verändern, nicht zwingend das Arbeitnehmernetto.", level: "Prüft Einstufung oder Mindestbemessungsgrundlage; die genaue Auswirkung hängt vom Land ab.", children: "Beeinflusst familien- oder kinderbezogene Steuer- und Sozialversicherungsregeln.", age: "Wird für altersabhängige Sozialversicherungsregeln benötigt.", withholding: "Offizieller individueller Einbehalt, der direkt auf der Abrechnung angewandt wird.", risk: "Richtwert der betrieblichen Risikoklasse; der tatsächliche Satz ist unternehmens- oder betriebsstättenspezifisch.", exact: "Ersetzt den Richtwert durch den exakten, dem Unternehmen mitgeteilten Prozentsatz.", profile: "Persönliches oder sozialversicherungsrechtliches Merkmal, das die Berechnung beeinflusst." },
  fr: { gross: "Brut annuel avant impôts et cotisations salariales ; ce n’est pas le coût total employeur.", geography: "Cette valeur détermine les impôts, surtaxes ou règles sociales applicables localement.", periods: "Modifie le montant de chaque paie, jamais le total annuel.", headcount: "Détermine les taux et seuils employeur liés à l’effectif.", contract: "Le type de contrat peut modifier les cotisations employeur, sans nécessairement changer le net salarié.", level: "Contrôle le classement ou l’assiette minimale ; l’effet exact dépend du pays.", children: "Modifie les règles fiscales ou sociales liées aux enfants et au foyer.", age: "Utilisé pour les règles sociales dépendant de l’âge.", withholding: "Retenue individuelle officielle appliquée directement sur la paie.", risk: "Classe de risque indicative ; le taux réel dépend de l’entreprise ou de l’établissement.", exact: "Remplace le scénario indicatif par le taux exact notifié à l’entreprise.", profile: "Caractéristique personnelle ou sociale qui modifie le calcul." },
  es: { gross: "Bruto anual antes de impuestos y cotizaciones del trabajador; no es el coste total empresarial.", geography: "Este valor determina los impuestos, recargos o reglas sociales aplicables localmente.", periods: "Cambia el importe de cada nómina, nunca el total anual.", headcount: "Determina tipos y umbrales empresariales vinculados al tamaño de la plantilla.", contract: "El tipo de contrato puede cambiar las cotizaciones empresariales sin modificar necesariamente el neto.", level: "Comprueba la clasificación o base mínima; el efecto exacto depende del país.", children: "Modifica reglas fiscales o sociales vinculadas a hijos y unidad familiar.", age: "Se utiliza para reglas de seguridad social dependientes de la edad.", withholding: "Retención individual oficial aplicada directamente en la nómina.", risk: "Clase de riesgo orientativa; el tipo real depende de la empresa o centro de trabajo.", exact: "Sustituye el escenario orientativo por el porcentaje exacto notificado a la empresa.", profile: "Característica personal o de seguridad social que modifica el cálculo." },
};

const FIELD_EXAMPLE_GENERIC: Record<Exclude<Locale, "it">, Record<"gross" | "periods" | "rate" | "selection", string>> = {
  en: {
...
const COUNTRY_NOTE: Record<Exclude<Locale, "it">, Record<EUCountry, readonly string[]>> = {
  en: {
    ...
    FR: ["Mutuelle, prévoyance and collective-agreement benefits are excluded.", "AT/MP and versement mobilité rates depend on the establishment."],
  },
  de: {
    ...
    FR: ["Mutuelle, prévoyance und tarifliche Leistungen sind ausgeschlossen.", "AT/MP und versement mobilité hängen von der Betriebsstätte ab."],
  },
  fr: {
    ...
    FR: ["Mutuelle, prévoyance et avantages conventionnels sont exclus.", "Les taux AT/MP et versement mobilité dépendent de l’établissement."],
  },
  es: {
    ...
    FR: ["Se excluyen mutuelle, prévoyance y beneficios de convenio.", "Los tipos AT/MP y versement mobilité dependen del centro."],
  },
};
...
function fieldExampleKind(input: InputDescriptor): "gross" | "periods" | "rate" | "selection" {
  if (input.field === "grossAnnual") return "gross";
  if (input.field === "payPeriods") return "periods";
